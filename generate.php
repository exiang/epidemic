...
function generateLatestStatusV1($date)
{
    ...
    // fonts
    $fontInterRegular = 'font/Inter-Regular.ttf';
    ...
    echo date('Y-m-d H:i', $dateTimestamp);
    exit;
    ...
    //
    // write date
    writeLeft($im, sprintf('%s.%s.%s', ltrim($dateDay, '0'), ltrim($dateMonth, '0'), $dateYear), 20, 58, 110, $colorBlack, $fontInterBold);

    // write accumulated case
    writeCenter($im, number_format($accumulatedCase), 48, 680, 170, 335, 67, $colorWhite, $fontInterRegular);

    //
    // write new case
    writeCenter($im, number_format($newCase), 53, 55, 328, 460, 90, imagecolorallocate($im, 238, 66, 43), $fontInterRegular);

    // write local case
    writeCenter($im, number_format($localCase), 35, 73, 465, 175, 50, $colorWhite, $fontInterRegular);

    // write import case
    writeCenter($im, number_format($importCase), 35, 320, 465, 175, 50, $colorWhite, $fontInterRegular);

    //
    // write new recover case
    writeCenter($im, number_format($newRecoverCase), 53, 567, 328, 460, 90, imagecolorallocate($im, 6, 61, 30), $fontInterRegular);

    // write accumulated recover case
    writeCenter($im, number_format($accumulatedRecoverCase), 35, 565, 465, 455, 50, $colorWhite, $fontInterRegular);

    //
    // write active case
    writeCenter($im, number_format($activeCase), 53, 55, 599, 460, 90, imagecolorallocate($im, 51, 27, 89), $fontInterRegular);

    // write icu case
    writeCenter($im, number_format($icuCase), 35, 73, 734, 175, 50, $colorWhite, $fontInterRegular);
    // write ventil case
    writeCenter($im, number_format($ventCase), 35, 320, 690, 175, 50, $colorWhite, $fontInterRegular);

    //
    // write new death case
    writeCenter($im, number_format($newDeathCase), 53, 567, 590, 460, 90, imagecolorallocate($im, 60, 34, 20), $fontInterRegular);
    // write accumulated death case
    writeCenter($im, number_format($accumulatedDeathCase), 35, 565, 740, 455, 50, $colorWhite, $fontInterRegular);

    // vaccination
    writeCenter($im, sprintf('(Sehingga %s)', $dateVax), 16, 800, 855, 210, 24, $colorWhite, $fontInterRegular);
    writeCenter($im, number_format($accumulatedVax), 35, 672, 982, 368, 40, $colorWhite, $fontInterRegular);

    // write partial vax percentage
    writeCenter($im, sprintf('%s %%', number_format((float)round($accumulatedPartialVax/$malaysiaPopulationArray['pop'] * 100, 2, PHP_ROUND_HALF_UP), 1, '.', ',')), 15, 382, 915, 63, 43, $colorWhite, $fontInterRegular);
    // write partial vax number
    writeCenter($im, sprintf('%s juta', number_format($accumulatedPartialVax/1000000, 1, '.', '')), 30, 465, 918, 160, 43, $colorWhite, $fontInterRegular);
    ;
    // write full vax percentage
    writeCenter($im, sprintf('%s %%', number_format((float)round($accumulatedFullVax/$malaysiaPopulationArray['pop'] * 100, 2, PHP_ROUND_HALF_UP), 1, '.', ',')), 15, 382, 975, 63, 43, $colorWhite, $fontInterRegular);
    ;
    // write full vax number
    writeCenter($im, sprintf('%s juta', number_format($accumulatedFullVax/1000000, 1, '.', '')), 30, 465, 978, 160, 43, $colorWhite, $fontInterRegular);
